...
import 'Rest2.dart';
import 'upperThree.dart'; // Import the next screen file

class UpperTwo extends StatefulWidget {
  const UpperTwo({super.key});

  @override
  _UpperTwoState createState() => _UpperTwoState();
}

class _UpperTwoState extends State<UpperTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/upper/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => UpperThree()),
    );
  }
