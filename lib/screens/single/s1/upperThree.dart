...
import 'Rest3.dart';
import 'upperFour.dart'; // Import the next screen file

class UpperThree extends StatefulWidget {
  const UpperThree({super.key});

  @override
  _UpperThreeState createState() => _UpperThreeState();
}

class _UpperThreeState extends State<UpperThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/upper/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => UpperFour()),
    );
  }
