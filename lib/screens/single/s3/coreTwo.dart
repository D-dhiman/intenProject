...
import 'Rest2.dart';
import 'coreThree.dart'; // Import the next screen file

class CoreTwo extends StatefulWidget {
  const CoreTwo({super.key});

  @override
  _CoreTwoState createState() => _CoreTwoState();
}

class _CoreTwoState extends State<CoreTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/core/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => CoreThree()),
    );
  }
