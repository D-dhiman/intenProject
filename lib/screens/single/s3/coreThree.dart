...
import 'Rest3.dart';
import 'coreFour.dart'; // Import the next screen file

class CoreThree extends StatefulWidget {
  const CoreThree({super.key});

  @override
  _CoreThreeState createState() => _CoreThreeState();
}

class _CoreThreeState extends State<CoreThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/core/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => CoreFour()),
    );
  }
