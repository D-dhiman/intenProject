...
import 'Rest2.dart';
import 'lowerThree.dart'; // Import the next screen file

class LowerTwo extends StatefulWidget {
  const LowerTwo({super.key});

  @override
  _LowerTwoState createState() => _LowerTwoState();
}

class _LowerTwoState extends State<LowerTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/lower/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => LowerThree()),
    );
  }
