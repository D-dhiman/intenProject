...
import 'Rest3.dart';
import 'lowerFour.dart'; // Import the next screen file

class LowerThree extends StatefulWidget {
  const LowerThree({super.key});

  @override
  _LowerThreeState createState() => _LowerThreeState();
}

class _LowerThreeState extends State<LowerThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/lower/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => LowerFour()),
    );
  }
