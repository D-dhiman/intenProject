...
import 'Rest2.dart';
import 'backThree.dart'; // Import the next screen file

class BackTwo extends StatefulWidget {
  const BackTwo({super.key});

  @override
  _BackTwoState createState() => _BackTwoState();
}

class _BackTwoState extends State<BackTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/back/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => BackThree()),
    );
  }
