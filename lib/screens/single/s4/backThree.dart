...
import 'Rest3.dart';
import 'backFour.dart'; // Import the next screen file

class BackThree extends StatefulWidget {
  const BackThree({super.key});

  @override
  _BackThreeState createState() => _BackThreeState();
}

class _BackThreeState extends State<BackThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/back/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => BackFour()),
    );
  }
