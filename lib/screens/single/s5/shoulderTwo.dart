...
import 'Rest2.dart';
import 'shoulderThree.dart'; // Import the next screen file

class ShoulderTwo extends StatefulWidget {
  const ShoulderTwo({super.key});

  @override
  _ShoulderTwoState createState() => _ShoulderTwoState();
}

class _ShoulderTwoState extends State<ShoulderTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/single/shoulder/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => ShoulderThree()),
    );
  }
