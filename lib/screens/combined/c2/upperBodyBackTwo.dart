...
import 'Rest2.dart';
import 'upperBodyBackThree.dart'; // Import the next screen file

class UpperBodyBackTwo extends StatefulWidget {
  const UpperBodyBackTwo({super.key});

  @override
  _UpperBodyBackTwoState createState() => _UpperBodyBackTwoState();
}

class _UpperBodyBackTwoState extends State<UpperBodyBackTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/upperBodyBack/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => UpperBodyBackThree()),
    );
  }
