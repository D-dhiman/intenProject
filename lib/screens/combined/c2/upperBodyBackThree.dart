...
import 'Rest3.dart';
import 'upperBodyBackFour.dart'; // Import the next screen file

class UpperBodyBackThree extends StatefulWidget {
  const UpperBodyBackThree({super.key});

  @override
  _UpperBodyBackThreeState createState() => _UpperBodyBackThreeState();
}

class _UpperBodyBackThreeState extends State<UpperBodyBackThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/upperBodyBack/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => UpperBodyBackFour()),
    );
  }
