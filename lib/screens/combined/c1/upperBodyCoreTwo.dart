...
import 'Rest2.dart';
import 'upperBodyCoreThree.dart'; // Import the next screen file

class UpperBodyCoreTwo extends StatefulWidget {
  const UpperBodyCoreTwo({super.key});

  @override
  _UpperBodyCoreTwoState createState() => _UpperBodyCoreTwoState();
}

class _UpperBodyCoreTwoState extends State<UpperBodyCoreTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/upperBodyCore/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => UpperBodyCoreThree()),
    );
  }
