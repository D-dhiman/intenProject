...
import 'Rest3.dart';
import 'fullBodySeatedFour.dart'; // Import the next screen file

class FullBodySeatedThree extends StatefulWidget {
  const FullBodySeatedThree({super.key});

  @override
  _FullBodySeatedThreeState createState() => _FullBodySeatedThreeState();
}

class _FullBodySeatedThreeState extends State<FullBodySeatedThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/fullBodySeated/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => FullBodySeatedFour()),
    );
  }
