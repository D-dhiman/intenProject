...
import 'Rest2.dart';
import 'fullBodySeatedThree.dart'; // Import the next screen file

class FullBodySeatedTwo extends StatefulWidget {
  const FullBodySeatedTwo({super.key});

  @override
  _FullBodySeatedTwoState createState() => _FullBodySeatedTwoState();
}

class _FullBodySeatedTwoState extends State<FullBodySeatedTwo> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/fullBodySeated/two');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => FullBodySeatedThree()),
    );
  }
