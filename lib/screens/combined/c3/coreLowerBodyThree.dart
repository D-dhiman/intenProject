...
import 'Rest3.dart';
import 'coreLowerBodyFour.dart'; // Import the next screen file

class CoreLowerBodyThree extends StatefulWidget {
  const CoreLowerBodyThree({super.key});

  @override
  _CoreLowerBodyThreeState createState() => _CoreLowerBodyThreeState();
}

class _CoreLowerBodyThreeState extends State<CoreLowerBodyThree> with SingleTickerProviderStateMixin {
  ...
  Future<void> fetchExerciseData() async {
    try {
      final url = Uri.parse('http://localhost:3000/exercises/coreLowerBody/three');
  ...
  void skipToNextScreen() {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => CoreLowerBodyFour()),
    );
  }
